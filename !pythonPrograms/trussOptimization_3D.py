...
def gather_node_info():
	...
	for node_info in all_node_info:
		...
		nly = node_info[2][1]
		nlz = node_info[2][2]
		if nsx == 1 and nsy == 1 and nsz == 1: #if node is free,
			if nlx == 0 and  nly == 0 and nlz == 0: #if node has no loads,
				select_node_info.append(node_info[0])
	genes = [0] * ( len(select_node_info)*3 + num_elements*2 )
...
def calculate_fitness(str_values):
	...
	# strain energy
#	s_nrg_avg = sum(str_values[16]) / len(str_values[16])
#	s_nrg_max = max(str_values[16])
#	uval = s_nrg_avg + s_nrg_max
#	ufit = 1 - (uval/uref)
...
#	ufit = ufit * 1.0
	...
	fitness = dfit + sfit + mfit # + ufit
	return fitness, dfit, sfit, mfit#, ufit
...
def insert_genes():
	...
	for ele_props in ele_values:
		...
		n1x = node1[0]  ; n1y = node1[1] ; n1z = node1[2]
		n2x = node2[0]  ; n2y = node2[1] ; n2z = node2[2]
	...
	for i in range(0, num_elements):
		gene_xcarea = genes[num_coord_genes + i]/500#reduction factor
...
def print_results():
	...
	print("ele_dia(cm), material code, n1 coord(m), n2 coord(m), FOS,fMode")
	for i in range(0, len(ele_values_p)):
		ele_props = ele_values_p[i]
		xc_area = ele_props[1]
		ele_dia = ( (4*xc_area/3.14159)**0.5 ) *100 #conv m to cm
		...
		n1_y = ele_props[0][0][0][1]
		n1_z = ele_props[0][0][0][2]
		n2_x = ele_props[0][1][0][0]
		n2_y = ele_props[0][1][0][1]
		n2_z = ele_props[0][1][0][2]
		print("% .6f" % ele_dia, mat_code,
			"% .3f" % n1_x, "% .3f" % n1_y, "% .3f" % n1_z,
			"% .3f" % n2_x, "% .3f" % n2_y, "% .3f" % n2_z,
			"% .3f" % fos, fm)
	print('total structure mass: '+str("%.3f" % total_mass)+' kg')
	print('max structural displacement: '+str("%.3f" % max_disp)+' mm')
	
def info_plotter():
	figTitle = 'Truss Optimization Using a Genetic Algorithm'
	...
	sub1.scatter(x, y_mfit, color='darkred', s=.5, marker ='.',
		label='mfit score')	
#	sub1.scatter(x, y_ufit, color='purple', s=.5, marker ='.',
#		label='ufit score')	
...
gene_range = 15 / 1000 # mm converted to meters
...
#str_values = designLibrary_test.cantilever_truss3D()
#str_values = designLibrary_test.cantileverBig_truss3D()
#str_values = designLibrary_test.cantilever_2()
#str_values = designLibrary_test.transmissionTower1()
str_values = designLibrary_test.spaceFrameRoof()
str_values_ini = copy.deepcopy(str_values)
variable_nodes, genes = gather_node_info()
parent = trussAnalysis_3D.run_analysis(str_values)
trussAnalysis_3D.plot_results(parent, fig,1, 2, 1, "Original Design")

dref = max(parent[10])
mref = parent[13]

s_nrg_avg_p = sum(parent[16]) / len(parent[16])
s_nrg_max_p = max(parent[16])
uref = s_nrg_avg_p + s_nrg_max_p
#uref = s_nrg_max_p
...
y_dfit=[] ; y_sfit=[] ; y_mfit=[]# ; y_ufit = []
while True:
	num_mutations = random.randrange(1,max_mutations+1)
	i+=1
	if i % 10 == 0: # print status
		s_nrg_avg = sum(parent[16]) / len(parent[16])
		print(i, "% .4f" % parent_fitness, "% .3f" % parent[13], "% .3f" % s_nrg_avg, "% .3f" % min(parent[14]))
	genes, changes = mutate_genes()
	child = insert_genes()
	child = trussAnalysis_3D.run_analysis(child) ##
...
#	y_ufit.append(ufit)
	...
	if child_fitness >= parent_fitness:
		...
		parent = child[:]
		num_parents += 1
#	if i == 50000 or keyboard.is_pressed('ctrl+q'):
	if parent_fitness >= 1.5 or keyboard.is_pressed('ctrl+q'):
...
opti_mass = parent[13]
print_results()
trussAnalysis_3D.plot_results(parent, fig, 1, 2, 2, "Optimized Design")
info_plotter()
trussAnalysis_3D.plot_results(parent, info_fig, 2, 3, 6, "Optimized Design")
fig.tight_layout()
plt.show()
